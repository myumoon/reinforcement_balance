...
def auth():
    ...
    return LaunchAuthorization.issue(AuditVerdict._verified("a"*64,"d"*64),platform.verified(),SaveVerdict._verified("attempt","e"*64,"b"*64))

def test_authorization_cannot_be_self_attested():
    with pytest.raises(ValueError): LaunchAuthorization("a"*64,"b"*64,"c"*64)
...
def test_launch_intent_and_activation_cardinality(tmp_path):
    ...
    lifecycle = lifecycle.reserve("run-1", "gameplay-1", "nonce-1", authorization=auth(),intent_log=tmp_path/"intent.jsonl")
    with pytest.raises(ValueError):
        lifecycle.reserve("run-2", "gameplay-2", "nonce-2", authorization=auth(),intent_log=tmp_path/"intent.jsonl")
...
def test_prelaunch_and_uncertain_failures_do_not_mix_with_outcomes(tmp_path):
    ...
    gate = LaunchLifecycle.begin("b").reserve("r", "g", "n", authorization=auth(),intent_log=tmp_path/"intent.jsonl")
    ...
    uncertain = LaunchLifecycle.begin("c").reserve("r2", "g2", "n2", authorization=auth(),intent_log=tmp_path/"intent.jsonl")
...
def test_launch_intent_requires_successful_durable_commit(tmp_path):
    with pytest.raises(ValueError):
        LaunchLifecycle.begin("a").reserve("r","g","n",authorization=auth(),intent_log=None)
    directory=tmp_path/"directory"; directory.mkdir()
    with pytest.raises(ValueError):
        LaunchLifecycle.begin("a").reserve("r","g","n",authorization=auth(),intent_log=directory)
