...
def get_unlocked_weapon_ids(stage_key: str) -> list[int]:
    """stage_key までにアンロックされた全 weapon_id を返す。"""
    order = _STAGE_KEY_TO_ORDER[stage_key]
    return [e.weapon_id for e in WEAPON_UNLOCK_ORDER if e.unlock_order <= order]


def get_unlocked_startable_weapon_ids(stage_key: str) -> list[int]:
...
def get_added_weapon_id(stage_key: str) -> int:
...
def build_weapon_params_for_cell(
    *,
    ...
    if pool_policy == "target_plus_anchor":
        ...
        raise ValueError(f"Unknown pool_policy: {pool_policy!r}. MVP supports only 'target_plus_anchor'.")
