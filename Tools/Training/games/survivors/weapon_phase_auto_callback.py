...
from games.survivors.state_modules import WeaponPhaseAutoStateModule, _WEAPON_AUTO_STATUS_FILENAME


class WeaponPhaseAutoCallback(BaseCallback):
    """WeaponPhaseAutoStateModule の SB3 コールバックラッパー。

    - _on_training_start: status ファイルから状態復元 → 停滞タイマーリセット → 初回 set_params
    ...
    def __init__(
        ...
        Args:
            ...
            output_dir: status ファイルの保存先ディレクトリ（同一run resume 用）。
            checkpoint_dir: branch resume 時の読み取り元ディレクトリ。
    ...
    def _on_training_start(self) -> None:
        # (a) 同一run resume: output_dir の status から状態を復元
        _loaded = False
        status_file = os.path.join(self._output_dir, _WEAPON_AUTO_STATUS_FILENAME) if self._output_dir else ""
        if status_file and os.path.exists(status_file):
            try:
                with open(status_file) as f:
                    state = json.load(f)
                self._module.import_state(state)
                _loaded = True
                print(
                    f"[WeaponPhaseAuto] resume 復元 (output_dir): "
                    f"weapon_phase={self._module.current_phase_key}"
                )
            except (json.JSONDecodeError, KeyError, OSError, ValueError):
                pass
        # (b) branch resume: checkpoint_dir から復元を試みる
        if not _loaded and self._checkpoint_dir:
            ckpt_file = os.path.join(self._checkpoint_dir, _WEAPON_AUTO_STATUS_FILENAME)
