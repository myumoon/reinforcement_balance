...
class ActionContract:
    ...
    _KEYS = frozenset({"schema_version", "physics_hz", "frame_skip", "decision_hz", "rows", "golden_fixture"})
...
def load_action_contract(path: Path=CONFIG):
    with path.open(encoding="utf-8") as f: return ActionContract.from_wire(yaml.safe_load(f))
