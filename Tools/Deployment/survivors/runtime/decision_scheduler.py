...
class ScreenDecisionScheduler:
    """capture cadence の全 frame で screen transition と UI policy を評価する。

    やさしい説明: combat の 15 Hz deadline を共有せず、30 Hz で届いた画面変化を毎回見ます。
    """

    def __init__(self) -> None:
        """評価数と直前 screen state を初期化する。

        やさしい説明: 時間 gate を持たないため、frame backlog を作りません。
        """
    ...
    def evaluation_count(self) -> int:
        """評価した capture frame 数を返す。

        やさしい説明: 15 Hz combat tick より多く評価されたことを telemetry/test で確認できます。
        """
        return self._evaluation_count

    @property
    def transition_count(self) -> int:
        """screen state が変わった回数を返す。

        やさしい説明: 初回 frame は遷移と数えず、二 frame 目以降の変化だけを数えます。
        """
    ...
    def evaluate(
        ...
        """一 frame の transition を記録し共有 UI policy の結果を返す。

        やさしい説明: UI の判断規則は再実装せず、Common の単一実装へそのまま委譲します。
        """
        if not isinstance(policy_input, UiPolicyInputV1):
            raise ValueError("policy_input must be UiPolicyInputV1")
        if not isinstance(config, NonModelUiPolicyConfigV1):
            raise ValueError("config must be NonModelUiPolicyConfigV1")
        if self._last_state is not None and policy_input.screen_state != self._last_state:
            self._transition_count += 1
        self._last_state = policy_input.screen_state
        self._evaluation_count += 1
        return decide_non_model_ui_intent(policy_input, config)

    def reset(self) -> None:
        """直前 screen state だけを忘れる。

        やさしい説明: 新 run の次 frame を古い run からの transition と数えません。
        """
