...
MANIFEST_KEYS=frozenset({"schema_version","artifact_kind","target_audit_hash","target_audit_report"})
...
def validate_manifest(manifest:Mapping[str,Any]):
    ...
    report=manifest.get("target_audit_report")
    if not isinstance(report,Mapping) or set(report)!={"schema_version","target_hash","status"} or report.get("schema_version")!="target_audit.v1" or report.get("status")!="PASS": raise ValueError("real audit report required")
    if manifest.get("target_audit_hash")!=canonical_hash(report): raise ValueError("target audit parent identity mismatch")
