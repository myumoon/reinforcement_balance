...
def test_contract_dataclasses_are_exact_and_frozen() -> None:
    ...
    expected = {
        ...
        UiPresentationSnapshotV1: (
            "schema_hash", "snapshot_id", "frame_id", "parser_artifact_hash",
            "screen_state", "candidate_set_hash", "inventory_hash",
            "source_content_hash", "ui_state_key", "candidates", "buttons",
        ),
        PerceptionSnapshot: (
            "snapshot_id", "frame_id", "captured_ns", "parser_artifact_hash",
            "source_content_hash", "ui_state_key", "screen_state", "deploy_obs",
            "item_context", "choices", "ui_presentation", "diagnostics",
        ),
    }
...
def test_source_hash_quantizes_but_ui_key_ignores_continuous_values() -> None:
    ...
    under_quantum = _hud(
        cards=(ParsedCard(0, "whip", "weapon", 2, .99994, "ok", (100, 100, 500, 500)),),
        captured_ns=1_100_000_000,
    )
    moved = _hud(
        cards=(ParsedCard(0, "whip", "weapon", 2, .8, "ok", (110, 100, 510, 500)),),
        captured_ns=1_200_000_000,
    )
    ...
    same_time = build_ui_presentation_from_hud(
        _hud(cards=(ParsedCard(0, "whip", "weapon", 2, .99994, "ok", (100, 100, 500, 500)),)),
        (1000, 1000), snapshot_id="s", frame_id="f",
    )
...
def test_target_equivalence_requires_every_geometric_and_confidence_gate() -> None:
    ...
    assert is_equivalent_ui_target(old, good)
...
def test_fallback_cards_and_buttons_have_unique_semantics() -> None:
    ...
    assert [target.semantic_kind for target in snapshot.candidates] == [
        "fallback_reward", "fallback_reward", "item_card",
    ]
    assert [target.semantic_action for target in snapshot.buttons] == ["reroll", "ack_chest", "confirm"]
