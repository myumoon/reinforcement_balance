...
void FSurvivorsVandalierWeapon::OnLevelChanged(FWeaponLevel NewLevel)
{
	CacheParams();
}

void FSurvivorsVandalierWeapon::CacheParams()
{
...
FVector2D FSurvivorsVandalierWeapon::GetOrbitOrbPos(int32 OrbIdx) const
{
...
void FSurvivorsVandalierWeapon::Tick(float Dt)
{
	if (!Logic) return;
...
void FSurvivorsVandalierWeapon::StartBombing()
{
	const FPassiveEffects& PE = GetPassiveEffects();
...
void FSurvivorsVandalierWeapon::SpawnBombShot(int32 OrbIdx)
{
	if (!Logic) return;
