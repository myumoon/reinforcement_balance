...
void FSurvivorsWeaponVandalierLogic::OnLevelChanged(FWeaponLevel NewLevel)
{
	CacheParams();
}

void FSurvivorsWeaponVandalierLogic::CacheParams()
{
...
FVector2D FSurvivorsWeaponVandalierLogic::GetOrbitOrbPos(int32 OrbIdx) const
{
...
void FSurvivorsWeaponVandalierLogic::Tick(float Dt)
{
	if (!Logic) return;
...
void FSurvivorsWeaponVandalierLogic::StartBombing()
{
	const FPassiveEffects& PE = GetPassiveEffects();
...
void FSurvivorsWeaponVandalierLogic::SpawnBombShot(int32 OrbIdx)
{
	if (!Logic) return;
