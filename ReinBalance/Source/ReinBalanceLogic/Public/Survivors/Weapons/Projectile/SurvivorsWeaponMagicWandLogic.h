...
class REINBALANCELOGIC_API FSurvivorsWeaponMagicWandLogic : public FSurvivorsWeaponLogic
{
public:
	virtual void Tick(float Dt) override;
	virtual void OnLevelChanged(FWeaponLevel NewLevel) override;
	/** Magic Wandのburst/target fallbackをsandboxへ複製する。
	 * 初心者向け: 発射途中の状態を保ち、参照先Logicだけをcloneへ付け替える。 */
	virtual TUniquePtr<FSurvivorsWeaponLogic> CloneForPreview(
		FSurvivorsGameLogic* InLogic) const override
	{
		auto Clone = MakeUnique<FSurvivorsWeaponMagicWandLogic>(*this);
		Clone->Logic = InLogic;
		return Clone;
	}
